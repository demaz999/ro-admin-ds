 ...
 *
 * ## Такт 20: страница — канон «типовой страницы-таблицы админки»
 *
 * Шапка, блок идентичности, строка-цель и колонка действий закреплены правилом
 * для любой будущей таблицы админки — разбор и провенанс в `docs/naming.md`,
 * раздел «Такт 20: типовая страница-таблица админки (канон)». Вариант такта 10
 * снят.
 *
 ...
 * Заглушки: «Добавить», клик по строке и действия строки — обработчиков нет
 * намеренно.
 */
 ...
 * демо-набор до пагинации, поэтому число в подвале и разбивка по страницам
...
<template>
  ...
  <div class="flex min-w-0 flex-col">
    <!--
      Шапка — канон такта 20 (A): поиск 440 слева, «Добавить» справа, одна
      вертикальная ось. Счётчика в шапке нет и не будет — число живёт в подвале.
      Явный `items-center` в `class` — чтобы поле и кнопка держались одной оси и
      паддинги плашки сверху и снизу оставались симметричными; `gap-3` — зазор строки.
    -->
    ...
    <Table attached-top attached data-slot="types-table">
      ...
      <TableRow v-for="(row, index) in rows" :key="row.id" interactive>
        ...
        <!--
          Наименование — блок идентичности (такт 20, B): эмблема 20 + зазор 8 +
          имя medium. Плоский текст, не ссылка — решение владельца такта 12.
        -->
        <TableCell variant="slot" :size="56" class="flex-1 px-4">
          <TableCellIdentity :icon="row.icon">
            {{ row.name }}
          </TableCellIdentity>
        </TableCell>
