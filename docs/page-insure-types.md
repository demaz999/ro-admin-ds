...
## Такт 23: колонка действий без заголовка

Решение владельца: у колонки действий заголовка нет. Текст прижимался к правому краю и висел над
зарезервированным вторичным слотом — карандаш стоит на 32 левее. Колонка и её ширина 200 остались,
заголовок пустой той же высоты, название ушло в `aria-label` (решение агента). Референс Mobbin 4/5 —
ссылки в разделе «Референсы Mobbin» выше.

## Такт 27: вторичный слот только при вторичных действиях

Решение владельца, 2026-09-15: резерв правого слота такта 22 (рекомендация чата) снят. Правила —
`docs/naming.md`, раздел «Действие строки — `TableRowAction` и `TableRowActions`».

| Дефолт, наведение на строку | Демо `?actions=demo` |
|---|---|
| ![Только карандаш у правого края](page-insure-types-action-hover.png) | ![Карандаш и вторичный слот](page-insure-types-actions-demo.png) |

### Что изменилось

| Что | Было (такт 22) | Стало | Провенанс |
|---|---|---|---|
| **вторичный слот** | резервировался всегда, пустой на странице без вторичных | рендерится только на странице со вторичными действиями; у строки без вторичных на такой странице — пустой резерв (`reserveSecondary`), чтобы карандаш держал общую X | решение владельца; `reserveSecondary` — решение сборки: демо смешивает наборы, а X внутри страницы одна |
| **карандаш в дефолте** | X 1339, правее — пустые 32 | у правого края ячейки: правый край слота карандаша на одной вертикали с правым краем «Добавить», оба на паддинге 16 | решение владельца |
| **ширина колонки** | `TABLE_ROW_ACTIONS_COLUMN` = 200 на всю админку | `tableRowActionsColumn(действия страницы)`: со слотом — 200; только карандаш — `TABLE_ROW_ACTIONS_COLUMN_EDIT_ONLY` = подпись 102.6 + 8 + 24 + паддинги 32 = 166.6, до кратного 4 — **168** | решение владельца; расчёт — замер агента |
| **X между страницами** | одна во всей админке | различается на ширину слота 32 — осознанная цена | решение владельца |

### Проверено после правок

Курсор, клики, Esc и снимки — headless Chrome через DevTools Protocol, окно 1440.

| Проверка | Результат |
|---|---|
| дефолт: правый край карандаша | слот карандаша 1391 = правый край «Добавить» 1391, дельта **0**; глиф — 1387 |
| дефолт: колонка | 168 в покое и при наведении, заголовок 168; вторичных слотов нет; X слота карандаша 1367 у всех 25 строк |
| дефолт: наведение | подпись «Редактировать» слева, `opacity` 0 → 1, до глифа **12** |
| демо: карандаш | X **1335** у всех строк — на 32 левее дефолта; колонка 200 |
| демо: слот | 24×24 через 8 правее карандаша у каждой строки; правый край слота = правый край «Добавить»; строка 1 — пустой резерв, строка 2 — «Копировать», строка 3 — кебаб |
| демо: «Копировать» | по наведению тултип «Копировать» |
| демо: кебаб | клик открывает «Копировать, Архивировать, Экспортировать», за линией — «Удалить»; плашка в пределах таблицы; закрывается по Esc и кликом мимо |
| «Статусы» | тот же дефолт: слот карандаша 1391 = «Добавить» 1391, колонка 168, подпись до глифа 12 |
| `/compare` | 114 иконок, 361 текстовый узел, провалов нет |
...
Долг переноса, закрывается по правилу «источник главнее»; решения владельца отмечены. Правила
целиком — `docs/naming.md`, «Всплывающая плашка — `Popover` на Reka» и «F. Пустой результат
поиска».

### Кебаб — Reka Popover вместо контейнера без Reka

Компонент `Popover` пересобран на примитивах Reka (`PopoverRoot`/`Trigger`/`Portal`/`Content`),
`TableRowActions` перешёл на новый API без изменения поведения страницы.

| Проверка | Результат |
|---|---|
| положение плашки до/после | X 1367, колонка та же — замер `align="end"` `:side-offset="4"` даёт **тот же прямоугольник**, что ручной `absolute top-7 right-0` (дельта 0px по всем четырём сторонам) |
| Enter с клавиатуры | открывает (реальный фокус окна, курсор через `computer`, не синтетический `dispatchEvent`) |
| Esc | закрывает; `document.activeElement` после закрытия — кнопка кебаба |
| клик мимо | закрывает (реальный `pointerdown` через CDP) |
| повторный клик по триггеру | закрывает открытую плашку, не открывает её заново |
| `aria-expanded` на триггере | `false` → `true` → `false` вслед за состоянием |
| портал | плашка — прямой ребёнок `body`, `role="dialog"` |
| высота окна 400, строка у нижнего края | `data-side="top"`, плашка целиком внутри окна (была бы `data-side="bottom"` и обрезана) |

### Прочие потребители `Popover` — положение и поведение не изменились

Замеры — дельта до/после такта 28, headless Chrome, окно 1440.

| Потребитель | Было (такт ≤27) | Стало (такт 28) |
|---|---|---|
| `TableFooter`, размер страницы | плашка на 8 выше кнопки, правый край вровень | то же: `side="top"` `align="end"` `:side-offset="8"`, дельта 0 |
| «Моих осмотров», массовые действия | плашка на 8 ниже связки чекбокс+кнопка, левый край вровень с чекбоксом | то же: `PopoverAnchor` оборачивает связку, `align="start"` `:side-offset="8"`, дельта 0 |
| «Моих осмотров», «Ещё» | плашка на 8 ниже кнопки, правый край вровень | то же: `align="end"` `:side-offset="8"`, дельта 0 |
| `FilterChip`, значения | плашка на 8 ниже чипа, левый край вровень | то же в пределах 2px (см. `docs/naming.md` — источник расхождения: `align="start"` меряет от триггера, не от чипа впритык) |

Побочно найден и исправлен дефект сборки: `PopoverTrigger`/`PopoverAnchor` с собственным
`data-slot` перебивали `data-slot="chip"` у обёрнутого `Chip` через `as-child` — счётчиковые
фильтр-чипы теряли маркер. Оба файла лишились своего `data-slot`, разбор — в их шапках.

### Пустой результат поиска

`?q=zzz` на `/insure-types`: тело таблицы показывает `TableEmptySearch` («Ничего не найдено» /
«Измените запрос или сбросьте поиск» / «Сбросить поиск»), подвал скрыт, таблица сама несёт нижнее
скругление (`:attached="total > 0"`). «Сбросить поиск» возвращает «1 – 25 из 50» и очищает поле.

### `Dialog`/`Lightbox` — предупреждения Reka

На витрине и `/compare` (обе страницы, где живут демонстрации `Dialog`/`Lightbox`) консоль чистая:
`trapFocus` стал явным пропом (`DialogContent.vue`, было — проезжий атрибут, Vue ругался на
`Extraneous non-props attributes`), `title`/`description` уходят в `VisuallyHidden`, когда у окна
нет своего видимого заголовка. Разбор — `app/components/ui/dialog/index.ts`.

### `/compare` — автопроверки и счётчик текстовых узлов

114 иконок, провалов нет. Текстовых узлов **365** (было 361, `+4`): скрытые заголовок и описание
второго диалога `/compare` («Схема со скроллом», раньше без обоих) — **+2**; скрытое описание
`Lightbox` — на `/compare` он в кадре дважды (обычный размер и увеличенный), поэтому **+2**, не
+1. Заголовок `Lightbox` уже существовал до такта 28 (подпись кадра), в счёт не идёт. Провалов
нет ни у одного из четырёх.
