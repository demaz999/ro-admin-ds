...
| 3 | Та же ошибка в комментариях `tailwind.css`; утилиты `bg-palette-0{2..5}-hover`/`-pressed` не заведены, хотя переменные есть | следствие п. 2 | тема не тронута — правка по отдельной команде |
...
| 6 | `[data-theme="atom"]` переопределяет `--shadow-elevated*`, а утилиты из `@theme inline` собраны литералом — переопределение не действует | дефект темы, визуально не проявляется | тема не тронута — ждёт команды; ловушка в `CLAUDE.md` |
