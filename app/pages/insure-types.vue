...
<template>
  ...
  <div class="flex min-w-0 flex-col">
    <!--
      ...
      продолжением строки поиска. Пробовал `items-baseline` через мердж
      `class` — выравнивание по базовой линии, а не вручную вычисленному
      отступу.

      Такт 17: `items-baseline` откачен владельцем. Базовая линия Input
      (40px, верх коробки близко к верху плашки) утянула весь ряд к верху
      76px-й плашки — Button, у которого нет своей текстовой базовой линии
      на первом уровне (она внутри вложенных узлов иначе), лёг по-другому,
      и три элемента разъехались по вертикали вместо одной оси. Возврат на
      `items-center` — тот же приём, что и в такте 13, просто добавлен явно
      через `class`, а не за счёт дефолта компонента (который менял
      `items-baseline`), чтобы диф в этом месте не терялся при следующей
      правке зазора. Зазор от поля — `gap-3` (12), как в такте 16.
    -->
