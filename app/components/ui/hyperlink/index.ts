 ...
 *
 * ## Ступень `lg` — решение владельца, а не замер
 *
 * В макете заголовок карточки набран 15/20 Bold, и вид `accent` перенёс именно
 * его. Заголовок карточки списка при этом читается вровень с подписями вокруг:
 * решением владельца от 2026-08-18 (такт 7) он поднят на **следующую ступень
 * шкалы кита — 16/20** (`--text-base`). Новая ступень не заводилась, взята
 * соседняя сверху; ниже неё в шкале только 15, выше 17/24.
 *
 * Ступень задаётся размером, а не видом: `accent` остаётся макетным 15/20, а
 * `size="lg"` — осознанным отклонением там, где ссылка работает заголовком
 * карточки. Запись — `docs/figma-fixes.md`, «Сознательные отклонения».
 *
 * ## Наведение
 *
...
export const hyperlinkVariants = cva(
  'inline-flex w-fit items-center outline-none transition-colors',
  {
    variants: {
      ...
      size: {
        /**
         * Заголовок карточки списка. Тактом 7 поднят с макетных 15/20 на 16/20,
         * тактом 11 — на **17/24**: владелец сверил обе ступени в отладке и
         * оставил вторую. Новых ступеней при этом не заведено, 17/24 в шкале
         * есть. Запись — `docs/figma-fixes.md`.
         */
        lg: 'text-lg',
        md: 'text-sm',
        sm: 'text-xs',
