...
| F | действия | **только карандаш**: подпись «Редактировать» по наведению → карандаш у правого края ячейки, вторичного слота нет; колонка 168. «Сделать копию» живёт в форме справочника. Клик по строке = редактирование, обработчиков нет намеренно | решение владельца, такт 26 (пункт F такта 24 снят); слот и ширина — такт 27 |
...
такт 26 его снял: копия статусной модели — действие формы справочника. С такта 27 слота нет,
правый край карандаша стоит на одной вертикали с правым краем «Добавить». Глиф для
...
| 5 | meztest | MezTest· |
| 6 | Тестовая модель 1 | Альфа Страхование, Сбер, Согаз |
| 7 | Тестовая модель 2 | Т-Страхование |
| 8 | Тестовая модель 3 | Т-Страхование |
| 9 | LOGO eng | Альфа Страхование |
...
| колонки | Наименование 240 · Компании 678 · действия 200 (такт 27: 240 · 710 · 168) |
...
| колонка действий | в каждой строке один глиф — карандаш, X **1339**; вторичный слот 24×24, детей нет; по наведению на строку подпись «Редактировать» видима (такт 27 — см. раздел ниже) |
...
| поиск «сбер» | 2 строки: Тестовая модель 1, Особый словарь; «1 – 2 из 2» |
...
## Такт 27: карандаш у правого края

Решение владельца, 2026-09-15: резерв вторичного слота такта 22 снят. У «Статусов» вторичных действий нет,
поэтому колонка — подпись и карандаш, ширина `TABLE_ROW_ACTIONS_COLUMN_EDIT_ONLY` = 168 (102.6 + 8 + 24 +
паддинги 32 = 166.6). Правила — `naming.md`, «Действие строки»; разбор с демо вторичных действий —
`page-insure-types.md`, «Такт 27».

| Проверка | Результат |
|---|---|
| правый край карандаша | слот карандаша 1391 = правый край «Добавить» 1391, дельта **0**; глиф — 1387 |
| колонка | 168 в покое и при наведении, заголовок 168; вторичных слотов нет; X слота карандаша 1367 у всех 17 строк |
| наведение | подпись «Редактировать» слева, до глифа **12** |
| колонки | Наименование 240 · Компании 710 · действия 168 |
| «Особый словарь» | две линии, строка 97 — как при колонке 200 |
| `?values=demo` | 9 линий, строка 377; центры имени, первой метки и карандаша — **28** |
| `/compare` | 114 иконок, 361 текстовый узел, провалов нет |
...
Долг переноса, закрывается по правилу «источник главнее»; правила целиком — `docs/naming.md`,
«Всплывающая плашка — `Popover` на Reka» и «F. Пустой результат поиска». На «Статусах»
вторичных действий у строки нет (такт 27), кебаба на странице не бывает — из потребителей
`Popover` здесь живёт только переключатель размера страницы в подвале.

| Проверка | Результат |
|---|---|
| переключатель размера страницы | `Popover` пересобран на Reka: `side="top"` `align="end"` `:side-offset="8"` — плашка на 8 выше кнопки, правый край вровень, как до такта (дельта 0) |
| `?q=zzz` | тело таблицы — `TableEmptySearch` («Ничего не найдено» / «Измените запрос или сбросьте поиск» / «Сбросить поиск»), подвала нет, таблица сама несёт нижнее скругление |
| «Сбросить поиск» | возвращает «1 – 17 из 17», поле пустое |
| консоль | ошибок и предупреждений Reka нет |

Нейтральная метка `Chip` (такт 26) в правках такта 28 не участвует: она не триггер и `Popover`
не открывает.
