...
- **Столбец «Состояния» выше — это употребление в компонентах, а не набор переменных.**
  ...
  один `default`» была ошибкой, поправлено сверкой такта 19 (подраздел «Сверка 2026-09-15» ниже).
- **`status-04` не используется ничем.** Переносим ради полноты палитры, но в мапинг-таблице
  помечаем как неупотребляемую.

### Сверка 2026-09-15 — дельта к 2026-08-11

Такт 19. Сверка 2026-08-11 (выше и мапинг-таблица раздела 7) не повторяется: сверено только то,
что изменилось с тех пор в теме или в файле кита. Способ — снимок `tailwind.css` на коммите
`934a399` (последний за 2026-08-11) против текущего плюс все переменные `mode` и `menu` через
Plugin API против значений раздела 7.

| Что | Результат |
|---|---|
| 78 переменных `mode` + `menu` в файле кита | **без изменений с 2026-08-11** — каждое значение совпадает с разделом 7 |
| 93 цветовых токена темы, существовавших 2026-08-11 | **без изменений с 2026-08-11** — ни одно значение не менялось, ни один токен не удалён |
| 24 токена, добавленных позже через `var()` (`field-*`, `list-*`, `row*`, `tag`, `dialog`, `accent-soft`, `popover-scroll-thumb`, `secondary-pressed`, `destructive-foreground`) | значения приходят из уже сверенных токенов; выбор слота — решение, записано комментарием в теме |
| `--chip` `#d9e8fc`, добавлен в очереди 2 | совпадает: мастер `badge` `747:2464` привязан к `accent/surface_bright` `#d9e8fc` |
| `--border-soft` `#d9e8fc`, добавлен в такте 6 | значение совпадает, **источник вне `mode`**: узел `19601:29063` дашборда привязан к внешней переменной `border/primary` коллекции `tokens_final` — факт дописан к вопросу 14 |

Состояния рамп по факту `mode`: у 01 — пять (`default`, `hover`, `pressed`, `bg`, `disabled`),
у 02–05 — `default`, `hover`, `pressed`, у 06 — `default`, `hover`, `pressed`, `disabled`. Тема
совпадает с этим бит в бит. Верной была таблица полноты в `naming.md`; «только `default`» в
`design-debt.md`, в «Наблюдениях» выше и в вопросе 13 — ошибка, поправлена.

#### Расхождения, найденные сверкой

| # | Что | Категория | Что сделано |
|---|---|---|---|
| 1 | Витрина показывала 21 цвет из 119, «Радиусы» и «Типографика» пусты, hex не следовал переключателю темы — сборщик сравнивал `selectorText` целиком и читал значения с корня | дефект сборки (витрина) | исправлено; ловушка в `CLAUDE.md` |
| 2 | «У рамп 02–05 только `default`» в `design-debt.md`, вопросе 13, `figma-fixes.md` и «Наблюдениях» раздела 6; «полный набор» у 06 в `naming.md` — у 06 нет `bg` | дефект документа | исправлено во всех |
| 3 | Та же ошибка в комментариях `tailwind.css`; утилиты `bg-palette-0{2..5}-hover`/`-pressed` не заведены, хотя переменные есть | следствие п. 2 | исправлено, такт 19 ч. 2: комментарии по факту, утилиты `hover`/`pressed` для 02–05 заведены |
| 4 | `--border-soft` снят с переменной внешней библиотеки `tokens_final` | расхождение источника | вопрос 14 |
| 5 | `status-06/disabled` алиасит в ступень `/100`, как `status-01/bg`; `status-01/disabled` — в `/200` | непоследовательность кита 1 | `figma-fixes.md` §4 |
| 6 | `[data-theme="atom"]` переопределяет `--shadow-elevated*`, а утилиты из `@theme inline` собраны литералом — переопределение не действует | дефект темы, визуально не проявляется | исправлено, такт 19 ч. 2: утилита `shadow-elevated*` ссылается на `var(--shadow-elevated*)`, значения — в блоке темы; ловушка в `CLAUDE.md` |
| 7 | Вариант `type=with shadow` `643:3051` сета `left_menu` и эффект-стиль `menu_shadow` в коде не отражены, срез нигде не записан | срез матрицы без записи | записано здесь; разбор — отдельным тактом по сайдбару |
...
**Тёмная ступень служебных цветов — вне `mode`, такт 30.** В коллекции `mode` её нет, и это замер,
а не вкус: белый текст ни на одной служебной роли не дотягивает до 4.5:1 (максимум — 4.35 у
`service/error-pressed`), цветной текст на своей `*-surface` — 2.4–3.9. Ступень держит плашки поверх
кадра и текст на мягких заливках экрана свободной съёмки. Объявлена в обеих темах одними значениями
— `atom` не теряет плашки (прецедент такта 19, п. 6). Запрос завести ступень в `mode` —
`figma-fixes.md`, «Дыры палитры».

| Источник | Значение | Слот | Provenance |
|---|---|---|---|
| прототип VA-9265 v17, `.mark` | `#145a37` — 8.2:1 к белому | `--success-strong` *(расш.)* | **прототип v17, решение владельца 2026-09-23** |
| прототип VA-9265 v17, `.card.auto .mark` | `#8c5a0b` — 5.9:1 | `--warning-strong` *(расш.)* | **прототип v17, решение владельца 2026-09-23** |
| прототип VA-9265 v17, `.mark.rej` | `#96322e` — 7.5:1 | `--destructive-strong` *(расш.)* | **прототип v17, решение владельца 2026-09-23** |
...
#### Уровни и длительности, добавленные тактом 30

| Токен | Значение | Роль | Provenance |
|---|---|---|---|
| `--opacity-dimmed` | `var(--opacity-disabled-strong)` = 0.32 | приглушение чужих кадров при подсветке связи — спека VA-9265 §15.2 | **закрыто фактом**: 0.32 уже есть в ките (спека `Button` `45:175`); роль своя, значение общее |
| `--duration-flash` | `1.5s` | вспышка строки шага после перехода «Показать в структуре» — спека §15.3 | спека VA-9265 §15.3; длительность держит `StepRow`, решение владельца 7 |
| `--animate-step-flash` | `step-flash var(--duration-flash) ease-out` | кадры вспышки: `--secondary-hover` до 18%, затем прозрачно | прототип v17, `@keyframes found`; кольцо прототипа (28%) не перенесено — уровня 28% в ките нет |

#### Такт 34

| Токен | Значение | Роль | Provenance |
|---|---|---|---|
| `--container-side-panel` | `440px` | ширина боковой панели экрана VA-9265: панель структуры и панель полноэкранного просмотра, утилита `w-side-panel` | **решение владельца 2026-09-23**: ширина панели структуры §7 (прототип v17 `.work`); замер такта 34 — минимум 418 |
| `--duration-bind-flash` | `820ms` | вспышка нижней плашки «Распределено» | спека VA-9265 §11.3; длительность держит `FrameBindBar` по прецеденту `--duration-flash` |

#### Такт 35 — окно-карточка кита 1

| Токен | Значение | Роль | Provenance |
|---|---|---|---|
| `--radius-4xl` | `48px` | скругление окна-карточки, только верхний левый угол | кит 1, `card` `817:34525` |
| `--shadow-modal` | `0 4px 20px 0 #0e1e331a` | тень окна-карточки | кит 1, `card` `817:34525`: `DROP_SHADOW` 0/4/20 `#0e1e33` 10% |
| `--overlay-modal` | `#00000066` | подложка под окном-карточкой, одна на оба размещения | **решение владельца 2026-09-23** (вопрос 35): экран `SchemeDrawer` `30957:19948`, в `mode` переменной нет |
| `--container-modal-edge` | `642px` | ширина краевого размещения (`Sheet`) | кит 1, `card` `817:34525` |
| `--container-modal` | `600px` | ширина центрального окна | прототип VA-9265 `.mbox` — у кита 1 центрального размещения нет |
| `--container-modal-narrow` | `440px` | ширина окна прогресса | прототип VA-9265 `.wbox` |
| `--container-form-label` | `170px` | колонка подписи строки формы — `Field orientation="left" labelWidth="form"`, утилита `w-form-label` | **решение владельца 2026-09-23**, такт 36: прототип VA-9265 `.f-row` (`170px 1fr`); у мастера кита 1 `input` `720:11753` подпись слева по содержимому |
...
| 2026-09-23 | Тёмная ступень `*-strong` у success, warning, destructive — значениями прототипа VA-9265 | в `mode` ступени нет, белый текст на служебных ролях кита не проходит 4.5:1; решение владельца, такт 30 |
